...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_c, out_c, conf):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_c, out_c, kernel_size=conf.conv_kernel_size,
                      padding="same", stride=conf.stride),
            nn.BatchNorm2d(out_c),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_c, out_c, kernel_size=conf.conv_kernel_size,
                      padding="same", stride=conf.stride),
            nn.BatchNorm2d(out_c),
            nn.ReLU(inplace=True)
        )
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, in_c: int, out_c: int, conf: DotMap):
        ...
        self.pool = nn.MaxPool2d((conf.pool_upconv_kernel_size, conf.pool_upconv_kernel_size))
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_c: int, out_c: int, attention: bool, conf: DotMap):
        ...
        self.up = nn.ConvTranspose2d(
            in_c, out_c, kernel_size=conf.pool_upconv_kernel_size, padding=0, stride=conf.pool_upconv_kernel_size)
...
class UNet(nn.Module):
    ...
    def __init__(self, conf: DotMap, attention: bool = False, n_labels: int = 1):
        ...
        c_in = conf.input_channels
        self.input_height = conf.input_height
        self.input_width = conf.input_width

        channels = [layer.filters for layer in conf.encoders_decoders]
        channels.append(conf.bottleneck.filters)

        self.encoders = nn.ModuleList([])
        self.b = BottleNeck(channels[-2], channels[-1], conf.bottleneck)
        self.decoders = nn.ModuleList([])

        for i in range(conf.depth):
            if i == 0:
                self.encoders.append(EncoderBlock(
                    c_in, channels[i], conf.encoders_decoders[i]))
            else:
                self.encoders.append(EncoderBlock(
                    channels[i-1], channels[i], conf.encoders_decoders[i]))

        for i in range(conf.depth, 0, -1):
            self.decoders.append(DecoderBlock(
                channels[i], channels[i-1], attention, conf.encoders_decoders[i-1]))

        self.outputs = nn.Conv2d(
            conf.output_channels, n_labels, kernel_size=1, padding=0, stride=1)
    ...
    def center_trf(self):
        ...
        """
        center_height = self.input_height // 2
        center_width = self.input_width // 2
        return self.pixel_trf(center_height, center_width)
